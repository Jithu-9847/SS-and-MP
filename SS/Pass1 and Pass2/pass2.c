...
void main()
{
  ...
  fp3 = fopen("out.dat", "r");        //Access all the files needed
  fp4 = fopen("object.dat", "w");

  fscanf(fp3, "%s%s%s", label, opcode, operand);  //read first line from the intermediate file

  while (strcmp(opcode, "END") != 0)  //To find the length of the program
  {
    prevaddr = address;
